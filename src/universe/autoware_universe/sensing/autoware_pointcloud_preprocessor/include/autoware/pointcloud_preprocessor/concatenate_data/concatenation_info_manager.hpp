 ...
 * configuration data that can be stored in concatenated point cloud info
 * messages.
 */
 ...
 *
 * This structure contains timing constraints for the advanced concatenation
 * strategy, including minimum and maximum reference timestamps that define the
 * time window for point cloud matching and concatenation.
 */
struct StrategyAdvancedConfig : public StrategyConfig {
  /**
   * @brief Construct configuration with timestamp constraints.
   * @param reference_timestamp_min Minimum reference timestamp for
   * concatenation window
   * @param reference_timestamp_max Maximum reference timestamp for
   * concatenation window
   */
  ...
  explicit StrategyAdvancedConfig(const std::vector<uint8_t> &serialized_data) {
    if (serialized_data.size() != sizeof(reference_timestamp_min_msg) +
                                      sizeof(reference_timestamp_max_msg)) {
      throw std::invalid_argument(
          "Invalid serialized data size for StrategyAdvancedConfig");
    }

    size_t offset = 0;

    // Deserialize reference_timestamp_min
    std::memcpy(&reference_timestamp_min_msg, serialized_data.data() + offset,
                sizeof(reference_timestamp_min_msg));
    offset += sizeof(reference_timestamp_min_msg);

    // Deserialize reference_timestamp_max
    std::memcpy(&reference_timestamp_max_msg, serialized_data.data() + offset,
                sizeof(reference_timestamp_max_msg));
  }
  ...
  [[nodiscard]] std::vector<uint8_t> serialize() const final {
    ...
    serialized.reserve(sizeof(reference_timestamp_min_msg) +
                       sizeof(reference_timestamp_max_msg));
 ...
 * including their status, indexing within the concatenated result, and
 * metadata.
 *
...
class ConcatenationInfoManager {
 public:
   ...
   *
   * @param matching_strategy_name Name of the matching strategy ("naive" or
   * "advanced")
   ...
   */
  ConcatenationInfoManager(const std::string &matching_strategy_name,
                           const std::vector<std::string> &input_topics)
      : concatenated_point_cloud_info_base_msg_(create_concatenation_info_base(
            matching_strategy_name, input_topics)),
        num_expected_sources_(
            concatenated_point_cloud_info_base_msg_.source_info.size()) {}
   ...
   *
   * @return Fresh ConcatenatedPointCloudInfo message with source topics
   * initialized
   */
  [[nodiscard]] autoware_sensing_msgs::msg::ConcatenatedPointCloudInfo
  reset_and_get_base_info() {
   ...
   *
   * Updates the source information for a specific topic with data from a point
   * cloud. This includes header information, status, and if the status is OK,
   * calculates the proper indexing (idx_begin and length) for the concatenated
   * result.
   *
  ...
  void update_source_from_point_cloud(
      ...
      autoware_sensing_msgs::msg::ConcatenatedPointCloudInfo
          &out_concatenated_point_cloud_info_msg) {
    auto [target_info, idx_begin] = find_source_info_and_next_idx(
        topic, out_concatenated_point_cloud_info_msg);

    target_info->header = source_cloud.header;
    target_info->status = status;
    if (status != autoware_sensing_msgs::msg::SourcePointCloudInfo::STATUS_OK)
      return;
   ...
   * This is useful when you have timing/frame information but no actual point
   * cloud data. Note that idx_begin and length are not updated with this
   * method.
   *
  ...
  void update_source_from_header(
      const std_msgs::msg::Header &header, const std::string &topic,
      uint8_t status,
      autoware_sensing_msgs::msg::ConcatenatedPointCloudInfo
          &out_concatenated_point_cloud_info_msg) {
    auto [target_info, idx_begin] = find_source_info_and_next_idx(
   ...
   * @param out_concatenated_point_cloud_info_msg Output message to update with
   * result
   */
  void set_result(const sensor_msgs::msg::PointCloud2 &concatenated_cloud,
                  autoware_sensing_msgs::msg::ConcatenatedPointCloudInfo
                      &out_concatenated_point_cloud_info_msg) const {
   ...
   * This is used to store parameters specific to the chosen concatenation
   * strategy.
   *
   * @param matching_strategy_config Serialized configuration data
   * @param out_concatenated_point_cloud_info_msg Output message to update
   */
  static void set_config(const std::vector<uint8_t> &matching_strategy_config,
                         autoware_sensing_msgs::msg::ConcatenatedPointCloudInfo
                             &out_concatenated_point_cloud_info_msg) {
    out_concatenated_point_cloud_info_msg.matching_strategy_config =
 ...
 private:
  ...
  /**
   * @brief Find source info structure and calculate next index for
   * concatenation.
   *
   ...
   * @param out_concatenated_point_cloud_info_msg Message containing source info
   * structures
   * @return SourceCloudInfo containing pointer to target info and calculated
   * index
  ...
  [[nodiscard]] SourceCloudInfo find_source_info_and_next_idx(
    ...
    if (!target_info) {
      throw std::runtime_error("Topic '" + topic +
                               "' not found in ConcatenatedPointCloudInfo");
    }

    if (target_info->idx_begin != 0 || target_info->length != 0) {
      throw std::runtime_error(
          "ConcatenatedPointCloudInfo already has source info for topic '" +
          topic + "'");
    }
  ...
  /**
   * @brief Create base concatenation info message with initialized source
   * topics.
   *
   * Creates and initializes the base concatenation info message with the
   * specified strategy and input topics. All source info structures are
   * initialized with default values and TIMEOUT status.
   *
  ...
  create_concatenation_info_base(
      const std::string &matching_strategy_name,
      const std::vector<std::string> &input_topics) const {
    autoware_sensing_msgs::msg::ConcatenatedPointCloudInfo
        concatenated_point_cloud_info_base;
    auto strategy_it = matching_strategy_name_map.find(matching_strategy_name);
    if (strategy_it == matching_strategy_name_map.end()) {
      throw std::invalid_argument("Unknown matching strategy: '" +
    ...
    for (const auto &topic : input_topics) {
      ...
      concatenated_point_cloud_info_base.source_info.emplace_back(
          std::move(info));
    }
